...
logitModel3_b <- glm(stroke~ age*hypertension + age*heart_disease +age*avg_glucose_level, data = beforeSMOTE, family = binomial())
logitModel4_b <- glm(stroke~ (age + avg_glucose_level)^2 , data = beforeSMOTE, family = binomial())
...
logitModel3_a <- glm(stroke~ age*hypertension + age*heart_disease +age*avg_glucose_level, data = afterSMOTE, family = binomial())
logitModel4_a <- glm(stroke~ (age +avg_glucose_level)^2, data = afterSMOTE, family = binomial())
logitModel5_a <- glm(stroke~ (age + glucoseGroup)^2, data = afterSMOTE, family = binomial())

#selecting model for furter analysis of code
logitModel_b <-logitModel1_b
logitModel_a <- logitModel1_a
...
cm_train <- confusionMatrix(beforeSMOTEpred, as.factor(test$stroke),positive="1" )
cm_train
```
After SMOTE
```{r results='markup'}
...
cm_train <- confusionMatrix(afterSMOTEpred, as.factor(testAfterSMOTE$stroke),positive="1" )
cm_train
```
...
Here we can observe the improvement in ROC and PR curve. Since we have significant number of 0 and 1 values.

```{r results='markup'}

list2<- SensitivityCutoffInfo( train = afterSMOTE, test = testDataAfterSmote, predict = "prediction", actual = "stroke" )
list2$plot
```