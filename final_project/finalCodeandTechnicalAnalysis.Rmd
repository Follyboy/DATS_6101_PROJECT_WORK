...
```
The same thing is observed for model2 with formula `r formula(rf2)`.\
...
Checking the precision of afterSMOTE model3 `r formula(rf3)`\
...
```
According to the precision values, model3 (`r formula(rf3)`) is performing best in random forest.\
...
Here with the upsampled datset the psuedo R2 value has increased a lot. 
...
So, the above results confirms that in cases with rare disease or imbalance, it is not a good idea to look at accuracy values. 
...
* we cannot upsample more datapoints in medical dataset so we will need to ask for more enteries with the disease
* or more significant terms for the model
